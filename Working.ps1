...
# Create a scheduled task to run the edit function every 3 minutes
try {
    Log-Message "Creating scheduled task for recurring edits"
    $action = New-ScheduledTaskAction -Execute "powershell.exe" -Argument "-ExecutionPolicy Bypass -Command '& { . $PSScriptRoot\create_user_and_ssh.ps1; Edit-SshdConfig -sshdConfigPath `"$sshdConfigPath`" }'"
